...
func New(cfg *config.Config) (*Daemon, error) {
	...
	if err != nil {
		otel.Error("recorder init failed", map[string]any{"error": err.Error()})
		return nil, err
	}
...
func (d *Daemon) Run() {
	otel.Info("started")
	otel.Info("hotkey registered", map[string]any{"hotkey": "Ctrl+Q"})
...
func (d *Daemon) startRecording() {
	if err := d.recorder.Start(); err != nil {
		otel.Error("recording failed", map[string]any{"error": err.Error()})
		return
	}
	otel.Info("recording started")
...
func (d *Daemon) stopRecording() {
	...
	otel.Info("transcribing")
	d.setState(StateTranscribing)

	audioPath, err := d.recorder.Stop()
	if err != nil {
		otel.Error("recording stop failed", map[string]any{"error": err.Error()})
...
func (d *Daemon) processRecording(audioPath string) {
	...
	if err != nil {
		otel.Error("transcription failed", map[string]any{"error": err.Error()})
		d.setState(StateIdle)
		return
	}
	...
	otel.Info("transcribed", map[string]any{"text": text, "chars": len(text), "seconds": duration})
